...
if ($_SESSION['rol'] === 'Administrador' || $_SESSION['rol'] === 'ConsultorVP') {
	...
	// LEFT OUTER JOIN sect SECT ON OP.nombre_concepto=SECT.id where estado = ('No participamos' || estado = 'Participamos')
	// ";
	$sql="SELECT OP.id,OP.fCreacion,OP.nomEntidad,OP.numContrato,OP.Objeto,OP.Presu,OP.ubicacion,OP.fPublicacion,OP.fcierre,SECT.nombre_concepto,OP.Consultor,OP.ejecutivo,OP.gerente,OP.estado,OP.MYSE,OP.link,OP.obser
	FROM oportunidades OP
    LEFT OUTER JOIN sect SECT ON OP.nombre_concepto=SECT.id";
} elseif ($_SESSION['rol'] === 'Govermment') {
	$sql = "SELECT OP.id,OP.fCreacion,OP.nomEntidad,OP.numContrato,OP.Objeto,OP.Presu,OP.ubicacion,OP.fPublicacion,OP.fcierre,SECT.nombre_concepto,OP.Consultor,OP.ejecutivo,OP.gerente,OP.estado,OP.MYSE,OP.link,OP.obser
	FROM oportunidades OP
  LEFT OUTER JOIN sect ST ON OP.nombre_concepto=SECT.id where SECT.id = '3' and (estado = 'No participamos' || estado = 'Participamos')
  ";
} elseif ($_SESSION['rol'] === 'Corporativo') {
	$sql = "SELECT OP.id,OP.fCreacion,OP.nomEntidad,OP.numContrato,OP.Objeto,OP.Presu,OP.ubicacion,OP.fPublicacion,OP.fcierre,SECT.nombre_concepto,OP.Consultor,OP.ejecutivo,OP.gerente,OP.estado,OP.MYSE,OP.link,OP.obser
	FROM oportunidades OP
    LEFT OUTER JOIN sect SECT ON OP.nombre_concepto=SECT.id where (SECT.id = '2' || SECT.id = '1') and (estado = 'No participamos' || estado = 'Participamos')
  ";
} elseif ($_SESSION['rol'] === 'Pymes') {
	$sql = "SELECT OP.id,OP.fCreacion,OP.nomEntidad,OP.numContrato,OP.Objeto,OP.Presu,OP.ubicacion,OP.fPublicacion,OP.fcierre,SECT.nombre_concepto,OP.Consultor,OP.ejecutivo,OP.gerente,OP.estado,OP.MYSE,OP.link,OP.obser
	FROM oportunidades OP
    LEFT OUTER JOIN sect SECT ON OP.nombre_concepto=SECT.id where SECT.id = '5' and ('No participamos' || estado = 'Participamos')
    ";
} elseif ($_SESSION['rol'] === 'Whosale') {
	$sql = "SELECT OP.id,OP.fCreacion,OP.nomEntidad,OP.numContrato,OP.Objeto,OP.Presu,OP.ubicacion,OP.fPublicacion,OP.fcierre,SECT.nombre_concepto,OP.Consultor,OP.ejecutivo,OP.gerente,OP.estado,OP.MYSE,OP.link,OP.obser
	FROM oportunidades OP
    LEFT OUTER JOIN sect SECT ON OP.nombre_concepto=SECT.id where SECT.id = '4' and ('No participamos' || estado = 'Participamos')
    ";
}
...
if (is_null(mysqli_fetch_array($query))) {
	echo "<script>
                ...
                window.location= '../../../administrador/oportunidadesOtros.php'
    </script>";
} else {
	$query = mysqli_query($con, $sql);
	$cel = 3;
